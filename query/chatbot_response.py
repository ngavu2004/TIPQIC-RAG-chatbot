...
def generate_chat_response(query: str, retrieved_docs) -> str:
    ...
    try:
        ...
        # Generate response
        response = llm.invoke(user_prompt)  # Gemini doesn't use system messages the same way
    ...
    try:
        ...
        if results:
            ...
            for i, (doc, score) in enumerate(results, 1):
                ...
                print(f"{i}. {os.path.basename(source)} (Page {page}) - Relevance: {score:.3f}")
                
        else:
            print("❌ No relevant documents found in the database.")
            print("💡 Try rephrasing your question or check if the documents are properly indexed.")
